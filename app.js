...
async function openPanel(acc) {
  ...
  saveBtn.textContent = 'Save Notes'; saveBtn.className = '';
...
async function saveNotes() {
  ...
  const saveBtn = document.getElementById('save-btn');
  saveBtn.textContent = 'Saving...'; saveBtn.className = '';
  const { error } = await sb.from('notes').upsert({ account_id: selectedId, note, updated_at: new Date().toISOString() });
  if (!error) {
    saveBtn.textContent = '✓ Saved'; saveBtn.className = 'saved';
  } else {
    saveBtn.textContent = 'Error — retry';
  }
  setTimeout(() => { saveBtn.textContent = 'Save Notes'; saveBtn.className = ''; }, 2000);
}
